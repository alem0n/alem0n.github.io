...
日常打ctf积累的ctf题，现在放到博客上，分享一下，制作匆忙还望各位英雄海涵。
...
![1541298107753](/images/2019-04-09-记录一些ctf杂项-2019//1541298107753.png)
...
题目中给了各项TTL的值，将数值转化为二进制取前两位即为他所传递的信息，然后将每四个值为一组拼接起来就是最后的信息。

![1554800071692](/images/2019-04-09-记录一些ctf杂项-2019/1554800071692.png)
...
for i in f.readlines():
    j = i.strip("\n").replace("TTL=","")
    j = '{:08b}'.format(int(j))  ##这里如果用bin()，则不会高位补零，这里是个坑
    if j != '':
        flag_bin.append(str(j)[:2])
f.close()

step = 4
flag_bins = [flag_bin[i:i+step] for i in range(0,len(flag_bin),step)]
##四个为一组
...
for i in flag_bins:
    ...
    for j in i:
        s = s + j
    flag_int = flag_int + chr(int('0b'+s,2))
    print(s)
print(flag_int)
...
![1554801368657](/images/2019-04-09-记录一些ctf杂项-2019/1554801368657.png)

最后的信息ffd8ff应该是jpg的文件头，拖入winhex

![1554801909194](/images/2019-04-09-记录一些ctf杂项-2019/1554801909194.png)

jpg文件无疑，这里保存成jpg发现只有一部分二维码

![1554801986827](/images/2019-04-09-记录一些ctf杂项-2019/1554801986827.png)

在winhex观察发现他一共包含了六个文件，分离出来

![1554802166959](/images/2019-04-09-记录一些ctf杂项-2019/1554802166959.png)

用ps合并然后扫描就可以了：

![1554802364656](/images/2019-04-09-记录一些ctf杂项-2019/1554802364656.png)
...
这里看密文和秘钥的关系应该是维吉尼亚密码，进行解密就ok了

![1554802478571](/images/2019-04-09-记录一些ctf杂项-2019/1554802478571.png)